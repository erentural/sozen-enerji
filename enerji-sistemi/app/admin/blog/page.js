...
import { FileText, PlusCircle, X, CheckCircle2, Clock, Image as ImageIcon, Eye, EyeOff } from "lucide-react";
...
export default function AdminBlogPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setFormData({ title: "", summary: "", content: "", imageUrl: "", published: true });
      ...
      <div className="bg-white dark:bg-slate-800/80 rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-100 dark:border-slate-700/80 backdrop-blur-sm">
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {posts.map((post) => (
              <div key={post.id} className="group border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden bg-slate-50 dark:bg-slate-900/50 hover:shadow-md transition-all">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-100 dark:border-slate-800 relative">
            <div className="sticky top-0 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md px-8 py-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center z-10">
              <h2 className="text-xl font-black text-slate-900 dark:text-slate-100">Yeni Blog & Haber Ekle</h2>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              ...
              <div className="flex items-center gap-3 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700">
                ...
                <label htmlFor="published" className="font-bold text-sm text-slate-700 dark:text-slate-300 cursor-pointer">Yazıyı hemen yayınla (İşareti kaldırırsanız taslak olarak kaydedilir)</label>
              </div>
