      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-4 space-y-2">
          <div className="bg-white dark:bg-slate-800/80 backdrop-blur-sm p-3 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700/80 space-y-1 transition-colors">
            <button onClick={() => setActiveTab("profile")} className={`w-full flex items-center gap-3 px-5 py-4 rounded-2xl text-sm font-bold transition-all ${activeTab === "profile" ? `${currentTheme.bg} text-white shadow-md` : "text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 hover:text-slate-900 dark:hover:text-slate-200"}`}><User className="w-5 h-5" /> Profil ve Güvenlik</button>
            <button onClick={() => setActiveTab("company")} className={`w-full flex items-center gap-3 px-5 py-4 rounded-2xl text-sm font-bold transition-all ${activeTab === "company" ? `${currentTheme.bg} text-white shadow-md` : "text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 hover:text-slate-900 dark:hover:text-slate-200"}`}><Building2 className="w-5 h-5" /> Kurumsal Bilgiler</button>
            <button onClick={() => setActiveTab("notifications")} className={`w-full flex items-center gap-3 px-5 py-4 rounded-2xl text-sm font-bold transition-all ${activeTab === "notifications" ? `${currentTheme.bg} text-white shadow-md` : "text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 hover:text-slate-900 dark:hover:text-slate-200"}`}><Bell className="w-5 h-5" /> Bildirim Tercihleri</button>
            <button onClick={() => setActiveTab("theme")} className={`w-full flex items-center gap-3 px-5 py-4 rounded-2xl text-sm font-bold transition-all ${activeTab === "theme" ? `${currentTheme.bg} text-white shadow-md` : "text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 hover:text-slate-900 dark:hover:text-slate-200"}`}><Palette className="w-5 h-5" /> Görünüm ve Tema</button>
          </div>
        </div>
        ...
        <div className="lg:col-span-8">
          <div className="bg-white dark:bg-slate-800/80 backdrop-blur-sm rounded-3xl p-8 shadow-sm border border-slate-100 dark:border-slate-700/80 transition-colors">
              ...
              <form onSubmit={handleSave} className="space-y-6 animate-in fade-in duration-300">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input type="email" value={profileForm.email} disabled className="w-full px-4 py-3 border border-slate-200 dark:border-slate-700/50 rounded-xl text-sm font-medium text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-slate-800/50 cursor-not-allowed" />
                  </div>
                  <div className="pt-4 border-t border-slate-100 dark:border-slate-700/80">
                    <h3 className="text-sm font-black text-slate-900 dark:text-slate-200 mb-4">Şifre Değiştirme</h3>
                    <div className="space-y-4">
                      <div><input type="password" placeholder="Mevcut Şifreniz" value={profileForm.currentPassword} onChange={(e) => setProfileForm({ ...profileForm, currentPassword: e.target.value })} className={`w-full px-4 py-3 border border-slate-200 dark:border-slate-600 rounded-xl text-sm bg-slate-50/50 dark:bg-slate-900/50 focus:outline-none ${currentTheme.focus} dark:text-slate-200`} /></div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input type="text" value={companyForm.phone} onChange={(e) => setCompanyForm({ ...companyForm, phone: e.target.value })} className={`w-full px-4 py-3 border border-slate-200 dark:border-slate-600 rounded-xl text-sm bg-slate-50/50 dark:bg-slate-900/50 focus:outline-none ${currentTheme.focus} dark:text-slate-200`} required />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-black text-slate-500 dark:text-slate-400 uppercase mb-2">Merkez Adres Bilgisi</label>
                    <textarea rows={3} value={companyForm.address} onChange={(e) => setCompanyForm({ ...companyForm, address: e.target.value })} className={`w-full px-4 py-3 border border-slate-200 dark:border-slate-600 rounded-xl text-sm bg-slate-50/50 dark:bg-slate-900/50 focus:outline-none ${currentTheme.focus} dark:text-slate-200 resize-none`} required></textarea>
                  </div>
                </div>
                <div className="pt-4 flex justify-end">
                  <button type="submit" className={`${currentTheme.bg} ${currentTheme.hoverBg} text-white font-bold px-8 py-3.5 rounded-xl flex items-center gap-2 text-sm`}><Save className="w-4 h-4" /> Kaydet</button>
            ...
            {activeTab === "notifications" && (
              <form onSubmit={handleSave} className="space-y-6 animate-in fade-in duration-300">
                <div><h2 className="text-xl font-black text-slate-900 dark:text-slate-200 mb-1">Bildirim Tercihleri</h2></div>
                <div className="h-px bg-slate-100 dark:bg-slate-700/80 w-full"></div>
                <div className="space-y-4">
                  {[
                    ...
                    <div key={item.key} className="flex items-center justify-between p-4 rounded-2xl border border-slate-100 dark:border-slate-700/80 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                      <div>
                        <h4 className="text-sm font-black text-slate-900 dark:text-slate-200">{item.title}</h4>
