...
export default function AdminSettingsPage() {
  ...
  // YENİ: Yedekleme / CSV İndirme Fonksiyonu
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-2">
          <div className="bg-white dark:bg-slate-800/80 backdrop-blur-sm p-3 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700/80 space-y-1 transition-colors">
            ...
            <button onClick={() => setActiveTab("smtp")} className={`w-full flex items-center gap-3 px-5 py-4 rounded-2xl text-sm font-bold transition-all ${activeTab === "smtp" ? `${currentTheme.bg} text-white shadow-md` : "text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50"}`}><Mail className="w-5 h-5" /> E-Posta Sunucusu</button>
            
            {/* YENİ: Yedekleme Butonu */}
        ...
        <div className="lg:col-span-8">
          <div className="bg-white dark:bg-slate-800/80 backdrop-blur-sm rounded-3xl p-8 shadow-sm border border-slate-100 dark:border-slate-700/80 transition-colors">
            ...
            {/* YENİ: VERİ VE YEDEKLEME SEKMESİ */}
            {activeTab === "backup" && (
            ...
            {/* DİĞER SEKMELER (Mevcut kodlar korundu) */}
            {activeTab === "profile" && (
              <form onSubmit={handleSaveSettings} className="space-y-6 animate-in fade-in duration-300">
                ...
                <div className="pt-4 flex justify-end"><button type="submit" className={`${currentTheme.bg} ${currentTheme.hoverBg} text-white font-bold px-8 py-3.5 rounded-xl flex items-center gap-2 text-sm`}><Save className="w-4 h-4" /> Kaydet</button></div>
              </form>
            )}
            ...
            {activeTab === "team" && (
              <div className="space-y-6 animate-in fade-in duration-300">
                <div className="flex justify-between items-center">
                  <div><h2 className="text-xl font-black text-slate-900 dark:text-slate-200 mb-1">Ekip ve Yetkiler</h2><p className="text-xs font-medium text-slate-500">Sisteme giriş yapabilecek personelleri yönetin.</p></div>
                  {!isAddingStaff && (<button onClick={() => setIsAddingStaff(true)} className={`${currentTheme.bg} text-white font-bold px-4 py-2 rounded-xl flex items-center gap-2 text-sm shadow-sm`}><Plus className="w-4 h-4" /> Yeni Ekle</button>)}
                </div>
                <div className="h-px bg-slate-100 dark:bg-slate-700/80 w-full"></div>
                ...
                <div className="space-y-3">
                  {staffList.length === 0 ? (<p className="text-sm text-slate-500 text-center py-6">Kayıtlı personel bulunmamaktadır.</p>) : (
                    staffList.map((staff) => (
                      <div key={staff.id} className="flex items-center justify-between p-4 border border-slate-100 dark:border-slate-700/80 rounded-2xl bg-white dark:bg-slate-800 hover:shadow-sm transition-all">
                        <div className="flex items-center gap-4">
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-white ${staff.role === 'ADMIN' ? 'bg-rose-500' : staff.role === 'SUPPORT' ? 'bg-blue-500' : 'bg-amber-500'}`}>{staff.name.charAt(0)}</div>
                          <div><p className="font-bold text-sm text-slate-900 dark:text-slate-200">{staff.name}</p><p className="text-xs text-slate-500 dark:text-slate-400">{staff.email}</p></div>
                        </div>
                        <div className="flex items-center gap-4">
                          <span className={`px-3 py-1 rounded-full text-xs font-bold border ${staff.role === 'ADMIN' ? 'bg-rose-50 text-rose-700 border-rose-200' : staff.role === 'SUPPORT' ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
                            {staff.role === 'ADMIN' ? 'Yönetici' : staff.role === 'SUPPORT' ? 'Destek' : 'Saha Ekibi'}
                          </span>
                          <button onClick={() => handleDeleteStaff(staff.id)} className="text-slate-400 hover:text-rose-500 transition-colors" title="Sil"><Trash2 className="w-5 h-5" /></button>
                        </div>
                      </div>
                    ))
                  )}
            ...
            {activeTab === "company" && (
              <form onSubmit={handleSaveSettings} className="space-y-6 animate-in fade-in duration-300">
                ...
                <div className="h-px bg-slate-100 dark:bg-slate-700/80 w-full"></div>
                <div className="space-y-4">
                  <div><label className="block text-xs font-black text-slate-500 uppercase mb-2">Şirket Resmi Unvanı</label><input type="text" value={companyForm.companyName} onChange={(e) => setCompanyForm({ ...companyForm, companyName: e.target.value })} className={`w-full px-4 py-3 border border-slate-200 rounded-xl text-sm bg-slate-50/50 focus:outline-none focus:ring-1 ${currentTheme.focus}`} required /></div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div><label className="block text-xs font-black text-slate-500 uppercase mb-2">Destek E-Postası</label><input type="email" value={companyForm.supportEmail} onChange={(e) => setCompanyForm({ ...companyForm, supportEmail: e.target.value })} className={`w-full px-4 py-3 border border-slate-200 rounded-xl text-sm bg-slate-50/50 focus:outline-none focus:ring-1 ${currentTheme.focus}`} required /></div>
                    <div><label className="block text-xs font-black text-slate-500 uppercase mb-2">Çağrı Merkezi / Telefon</label><input type="text" value={companyForm.phone} onChange={(e) => setCompanyForm({ ...companyForm, phone: e.target.value })} className={`w-full px-4 py-3 border border-slate-200 rounded-xl text-sm bg-slate-50/50 focus:outline-none focus:ring-1 ${currentTheme.focus}`} required /></div>
                  </div>
                  <div><label className="block text-xs font-black text-slate-500 uppercase mb-2">Merkez Adres Bilgisi</label><textarea rows={3} value={companyForm.address} onChange={(e) => setCompanyForm({ ...companyForm, address: e.target.value })} className={`w-full px-4 py-3 border border-slate-200 rounded-xl text-sm bg-slate-50/50 focus:outline-none focus:ring-1 ${currentTheme.focus} resize-none`} required></textarea></div>
                </div>
                <div className="pt-4 flex justify-end"><button type="submit" className={`${currentTheme.bg} ${currentTheme.hoverBg} text-white font-bold px-8 py-3.5 rounded-xl flex items-center gap-2 text-sm`}><Save className="w-4 h-4" /> Kaydet</button></div>
              </form>
            )}
            ...
            {activeTab === "schedule" && (
              <form onSubmit={handleSaveSettings} className="space-y-6 animate-in fade-in duration-300">
                <div><h2 className="text-xl font-black text-slate-900 dark:text-slate-200 mb-1">Çalışma ve Randevu Saatleri</h2></div>
                <div className="h-px bg-slate-100 dark:bg-slate-700/80 w-full"></div>
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div><label className="block text-xs font-black text-slate-500 uppercase mb-2">Mesai Başlangıç Saati</label><input type="time" value={scheduleForm.workHourStart} onChange={(e) => setScheduleForm({ ...scheduleForm, workHourStart: e.target.value })} className={`w-full px-4 py-3 border border-slate-200 rounded-xl text-sm bg-slate-50/50 focus:outline-none focus:ring-1 ${currentTheme.focus}`} required /></div>
                    <div><label className="block text-xs font-black text-slate-500 uppercase mb-2">Mesai Bitiş Saati</label><input type="time" value={scheduleForm.workHourEnd} onChange={(e) => setScheduleForm({ ...scheduleForm, workHourEnd: e.target.value })} className={`w-full px-4 py-3 border border-slate-200 rounded-xl text-sm bg-slate-50/50 focus:outline-none focus:ring-1 ${currentTheme.focus}`} required /></div>
                  </div>
                  <div className="pt-2"><label className="flex items-center gap-3 cursor-pointer p-4 rounded-2xl border border-slate-200 hover:bg-slate-50/50 transition-colors"><input type="checkbox" checked={scheduleForm.allowWeekend} onChange={(e) => setScheduleForm({ ...scheduleForm, allowWeekend: e.target.checked })} className="w-5 h-5 rounded text-blue-600 focus:ring-blue-500" /><div><span className="block font-bold text-sm text-slate-800 dark:text-slate-200">Hafta Sonu Randevularına İzin Ver</span><span className="block text-xs text-slate-500">İşaretlenirse Pazar günleri de randevu alınabilir.</span></div></label></div>
                </div>
                <div className="pt-4 flex justify-end"><button type="submit" className={`${currentTheme.bg} ${currentTheme.hoverBg} text-white font-bold px-8 py-3.5 rounded-xl flex items-center gap-2 text-sm`}><Save className="w-4 h-4" /> Kaydet</button></div>
              </form>
            )}
            ...
            {activeTab === "pdf" && (
              <form onSubmit={handleSaveSettings} className="space-y-6 animate-in fade-in duration-300">
                <div><h2 className="text-xl font-black text-slate-900 dark:text-slate-200 mb-1">PDF ve Belge Ayarları</h2></div>
                <div className="h-px bg-slate-100 dark:bg-slate-700/80 w-full"></div>
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div><label className="block text-xs font-black text-slate-500 uppercase mb-2">Vergi Numarası</label><input type="text" value={pdfForm.taxNumber} onChange={(e) => setPdfForm({ ...pdfForm, taxNumber: e.target.value })} className={`w-full px-4 py-3 border border-slate-200 rounded-xl text-sm bg-slate-50/50 focus:outline-none focus:ring-1 ${currentTheme.focus}`} /></div>
                    <div><label className="block text-xs font-black text-slate-500 uppercase mb-2">Mersis Numarası</label><input type="text" value={pdfForm.mersisNumber} onChange={(e) => setPdfForm({ ...pdfForm, mersisNumber: e.target.value })} className={`w-full px-4 py-3 border border-slate-200 rounded-xl text-sm bg-slate-50/50 focus:outline-none focus:ring-1 ${currentTheme.focus}`} /></div>
                  </div>
                  <div><label className="block text-xs font-black text-slate-500 uppercase mb-2">Rapor Alt Bilgisi (Footer Metni)</label><textarea rows={3} value={pdfForm.pdfFooterText} onChange={(e) => setPdfForm({ ...pdfForm, pdfFooterText: e.target.value })} className={`w-full px-4 py-3 border border-slate-200 rounded-xl text-sm bg-slate-50/50 focus:outline-none focus:ring-1 ${currentTheme.focus} resize-none`} required></textarea></div>
                </div>
                <div className="pt-4 flex justify-end"><button type="submit" className={`${currentTheme.bg} ${currentTheme.hoverBg} text-white font-bold px-8 py-3.5 rounded-xl flex items-center gap-2 text-sm`}><Save className="w-4 h-4" /> Kaydet</button></div>
              </form>
            )}
            ...
            {activeTab === "smtp" && (
              <form onSubmit={handleSaveSettings} className="space-y-6 animate-in fade-in duration-300">
                <div><h2 className="text-xl font-black text-slate-900 dark:text-slate-200 mb-1">E-Posta (SMTP) Ayarları</h2></div>
                <div className="h-px bg-slate-100 dark:bg-slate-700/80 w-full"></div>
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="md:col-span-2"><label className="block text-xs font-black text-slate-500 uppercase mb-2">Sunucu (Host)</label><input type="text" value={smtpForm.smtpHost} onChange={(e) => setSmtpForm({ ...smtpForm, smtpHost: e.target.value })} className={`w-full px-4 py-3 border border-slate-200 rounded-xl text-sm bg-slate-50/50 focus:outline-none focus:ring-1 ${currentTheme.focus}`} required /></div>
                    <div><label className="block text-xs font-black text-slate-500 uppercase mb-2">Port</label><input type="number" value={smtpForm.smtpPort} onChange={(e) => setSmtpForm({ ...smtpForm, smtpPort: e.target.value })} className={`w-full px-4 py-3 border border-slate-200 rounded-xl text-sm bg-slate-50/50 focus:outline-none focus:ring-1 ${currentTheme.focus}`} required /></div>
                  </div>
                  <div><label className="block text-xs font-black text-slate-500 uppercase mb-2">Gönderici E-Posta Adresi</label><input type="email" value={smtpForm.smtpUser} onChange={(e) => setSmtpForm({ ...smtpForm, smtpUser: e.target.value })} className={`w-full px-4 py-3 border border-slate-200 rounded-xl text-sm bg-slate-50/50 focus:outline-none focus:ring-1 ${currentTheme.focus}`} /></div>
                  <div><label className="block text-xs font-black text-slate-500 uppercase mb-2">Uygulama Şifresi</label><input type="password" value={smtpForm.smtpPass} onChange={(e) => setSmtpForm({ ...smtpForm, smtpPass: e.target.value })} className={`w-full px-4 py-3 border border-slate-200 rounded-xl text-sm bg-slate-50/50 focus:outline-none focus:ring-1 ${currentTheme.focus}`} /></div>
                </div>
                <div className="pt-4 flex justify-end"><button type="submit" className={`${currentTheme.bg} ${currentTheme.hoverBg} text-white font-bold px-8 py-3.5 rounded-xl flex items-center gap-2 text-sm`}><Save className="w-4 h-4" /> Kaydet</button></div>
              </form>
            )}
            ...
            {activeTab === "theme" && (
              <div className="space-y-6 animate-in fade-in duration-300">
                <div><h2 className="text-xl font-black text-slate-900 dark:text-slate-200 mb-1">Görünüm ve Tema</h2></div>
                <div className="h-px bg-slate-100 dark:bg-slate-700/80 w-full"></div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      {[{ id: "light", label: "Aydınlık", icon: Sun }, { id: "dark", label: "Karanlık", icon: Moon }, { id: "system", label: "Sistem", icon: Monitor }].map((item) => (
                        <div key={item.id} onClick={() => handleThemeChange("mode", item.id)} className={`cursor-pointer p-4 rounded-2xl border-2 flex flex-col items-center justify-center gap-2 ${themeForm.mode === item.id ? `${currentTheme.border} ${currentTheme.text} bg-slate-50` : "border-slate-100 text-slate-600 hover:border-slate-200"}`}>
