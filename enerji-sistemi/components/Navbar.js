      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        <div className="flex justify-between items-center h-24">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/teklif-al" className="hidden md:block bg-[#FFC107] text-gray-900 px-6 py-3 rounded font-bold hover:bg-yellow-500 transition-colors shadow-sm text-center">
              Fiyat Teklifi Al
            </Link>

            {/* Giriş / Portal Yönetimi */}
            {session ? (
              <Link 
                href={session.user.role === "ADMIN" ? "/admin" : "/portal"}
                className="flex items-center gap-2 bg-[#02529C] text-white px-5 py-3 rounded font-bold hover:bg-blue-800 transition-colors shadow-sm"
              >
