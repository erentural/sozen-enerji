...
import { useTheme } from "./ThemeContext"; // YENİ: Temayı içeri aktarıyoruz
...
export default function AdminDashboard() {
  ...
  const { currentTheme, themeForm } = useTheme(); // YENİ: Tema ayarlarını çek
  ...
  // Premium İstatistik Kartı Bileşeni - Karanlık Mod Destekli
  const StatCard = ({ title, value, icon: Icon, color, link, linkText }) => {
    const colorClasses = {
      ...
      yellow: "text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30 border-amber-100 dark:border-amber-800",
      red: "text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-900/30 border-rose-100 dark:border-rose-800",
      green: "text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 border-emerald-100 dark:border-emerald-800",
    };
    const bgHover = {
    ...
    return (
      <Link href={link} className="block group h-full">
        <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-100 dark:border-slate-700 relative overflow-hidden transition-all duration-500 hover:-translate-y-1.5 hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] dark:hover:shadow-[0_20px_40px_rgba(0,0,0,0.3)] h-full flex flex-col">
          ...
          <div className="flex justify-between items-start mb-6">
            <div className="relative z-10 pr-2">
              ...
              <h3 className="text-5xl font-black text-slate-900 dark:text-white tracking-tighter transition-colors">
  ...
  return (
    <div className="font-sans transition-colors duration-300">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
        <div>
          <h1 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white flex items-center gap-3 transition-colors">
        ...
        <div className="bg-white dark:bg-slate-800 px-5 py-3 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm flex items-center gap-3 transition-colors">
          <div className="relative flex h-3.5 w-3.5">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
            <span className="relative inline-flex rounded-full h-3.5 w-3.5 bg-emerald-500"></span>
          </div>
          <span className="text-sm font-bold text-slate-700 dark:text-slate-200">Sistem Aktif & Çevrimiçi</span>
        </div>
      </div>
      ...
      <div className="relative overflow-hidden rounded-[2.5rem] bg-slate-900 shadow-2xl transition-all duration-300">
        
        {/* Tema Rengine Göre Değişen Dinamik Gradient Katmanları */}
        ...
        <div className={`absolute bottom-0 left-0 -mb-20 -ml-20 w-96 h-96 bg-slate-600 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-blob animation-delay-2000 transition-colors duration-1000`}></div>
        
        <div className="relative z-10 p-10 md:p-14 lg:p-16 flex flex-col lg:flex-row items-center justify-between gap-12 border border-slate-700/50 rounded-[2.5rem]">
          ...
          <div className="max-w-2xl text-white">
            ...
            <p className="text-slate-300 text-lg leading-relaxed mb-10 font-medium max-w-xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <Link href="/" target="_blank" className="group bg-white/5 hover:bg-white/10 backdrop-blur-md border border-white/10 text-white px-8 py-4 rounded-xl font-bold transition-all flex items-center justify-center gap-2">
                Siteyi Görüntüle <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </Link>
            </div>
          </div>
          
          {/* Sağ Taraftaki Şık İkonografi (Temaya Göre Renklenir) */}
