  ...
  return (
    <div className="p-8 max-w-7xl mx-auto font-sans transition-colors duration-300">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          {/* text-white yerine göz yormayan text-slate-200 kullanıldı */}
        ...
        <div className="p-5 border-b border-slate-100 dark:border-slate-700/80 flex items-center gap-3 bg-slate-50/50 dark:bg-slate-900/50 transition-colors">
          <div className="relative flex-1 max-w-md">
            ...
            <input 
              ...
              className={`w-full pl-11 pr-4 py-3 border border-slate-200 dark:border-slate-600 rounded-2xl focus:outline-none focus:ring-1 ${currentTheme.focus} bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 text-sm font-medium transition-colors`}
            />
          </div>
        </div>
        ...
        {filteredMessages.length === 0 ? (
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-700/50">
            {filteredMessages.map((msg) => {
              ...
              let rowBg = "";
              if (msg.replied) rowBg = "bg-white dark:bg-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-700/30";
              else if (msg.read) rowBg = "bg-slate-50/50 dark:bg-slate-800/30 hover:bg-slate-100/50 dark:hover:bg-slate-700/50";
              else rowBg = `bg-slate-50 dark:bg-slate-800/80 border-l-4 ${currentTheme.border} hover:bg-slate-100 dark:hover:bg-slate-700/80`;
                ...
                <div key={msg.id} className={`p-6 transition-all flex flex-col xl:flex-row items-start xl:items-center justify-between gap-6 ${rowBg}`}>
                  ...
                  <div className="flex-1 space-y-3 w-full">
                    <div className="flex items-center gap-3 flex-wrap">
                      <h3 className={`text-lg font-black transition-colors ${!msg.read ? currentTheme.text : 'text-slate-900 dark:text-slate-200'}`}>
                        {msg.name}
                      </h3>
                      ...
                      <span className="bg-slate-100 dark:bg-slate-700/50 text-slate-700 dark:text-slate-300 font-bold px-3 py-1 rounded-full text-xs border border-slate-200 dark:border-slate-600/50 transition-colors">
                    ...
                    <p className={`text-sm p-4 rounded-xl border border-slate-100 dark:border-slate-700/50 shadow-sm transition-colors ${
                      !msg.read 
                  ...
                  <div className="flex flex-row xl:flex-col items-center xl:items-end gap-3 w-full xl:w-auto justify-between xl:justify-center border-t xl:border-t-0 pt-4 xl:pt-0 border-slate-100 dark:border-slate-700/50 transition-colors">
                    <span className="text-xs font-bold text-slate-400 dark:text-slate-500 text-right">
                      ...
                      <span className="text-[10px] bg-slate-100 dark:bg-slate-700/50 px-2 py-0.5 rounded-md mt-1 inline-block ml-2 xl:ml-0 transition-colors">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {!msg.read && (
                        <button 
                          ...
                          className="flex items-center gap-1.5 px-3 py-2 rounded-xl transition-colors text-sm font-bold shadow-sm bg-white dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-600/80 hover:bg-slate-50 dark:hover:bg-slate-700"
                      ...
                      <button 
                        ...
                        className={`flex items-center gap-1.5 px-3 py-2 rounded-xl transition-all text-sm font-bold shadow-sm ${
                          msg.replied 
                            ? 'bg-white dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-600/80 hover:bg-slate-50 dark:hover:bg-slate-700' 
      ...
      {isModalOpen && selectedMessage && (
        <div className="fixed inset-0 bg-slate-900/60 dark:bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-colors">
          <div className="bg-white dark:bg-slate-800/95 backdrop-blur-xl rounded-3xl shadow-2xl max-w-lg w-full p-6 relative animate-in fade-in zoom-in duration-200 border border-slate-100 dark:border-slate-700/80 transition-colors">
            ...
            <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-700/80 mb-6 transition-colors">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 italic bg-white dark:bg-slate-800/50 p-3 rounded-xl border border-slate-200 dark:border-slate-700/50 transition-colors shadow-sm">
            ...
            <form onSubmit={handleReply} className="space-y-4">
              <div>
                ...
                <textarea 
                  ...
                  className={`w-full px-4 py-3 border border-slate-200 dark:border-slate-600 rounded-2xl focus:outline-none focus:ring-1 ${currentTheme.focus} bg-white dark:bg-slate-900/50 text-slate-800 dark:text-slate-200 text-sm font-medium resize-none transition-colors`}
                ></textarea>
              </div>
